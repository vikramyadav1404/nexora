...
- **A reported media 401 in the browser I could not reproduce.** The server is verified end to end: `/api/auth/me` emits the cookie correctly given only an access token, and the proxy 302s through the client-domain rewrite. No stale URLs in the database, the service worker skips `/api`, and the boot sequence cannot race. Cause unestablished. Mitigated, not fixed — a media 401 now re-mints the cookie once and retries rather than rendering as a permanently broken image.
