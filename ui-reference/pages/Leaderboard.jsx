    ...
    <div className="page-container">
      <div style={{ maxWidth: 700, margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          ...
          <h1 style={{ fontSize: 32, fontFamily: 'var(--font-heading)', marginBottom: 8 }}>
            <span className="text-gradient">Leaderboard</span>
          </h1>
          <p style={{ color: 'var(--text-muted)' }}>Top contributors ranked by points earned</p>

          {/* Your rank */}
          {user && (
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, marginTop: 16, padding: '10px 20px', background: 'rgba(124,58,237,0.1)', border: '1px solid rgba(124,58,237,0.3)', borderRadius: 'var(--radius-full)' }}>
              <Star size={16} style={{ color: 'var(--accent)' }} />
              <span style={{ fontSize: 14 }}>Your points: <strong style={{ color: 'var(--accent)', fontSize: 18 }}>{user.points || 0}</strong></span>
            </div>
          )}
        </div>
                ...
                leaderboard.map((person, i) => {
                  ...
                  return (
                    <div key={person._id}
                      ...
                      style={{ cursor: 'pointer', background: isCurrentUser ? 'rgba(124,58,237,0.08)' : 'transparent', borderRadius: 'var(--radius-md)' }}
                      onClick={() => navigate(`/profile/${person._id}`)}>
                      {/* Rank */}
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 600, fontSize: 15, display: 'flex', alignItems: 'center', gap: 6 }}>
                          {person.name}
                          {isCurrentUser && <span style={{ fontSize: 11, color: 'var(--primary-light)', background: 'rgba(124,58,237,0.15)', padding: '1px 8px', borderRadius: 'var(--radius-full)' }}>You</span>}
                        </div>
