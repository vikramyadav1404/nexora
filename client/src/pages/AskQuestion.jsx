...
import { ArrowLeft, Send, Tag, HelpCircle, AlertCircle, Sparkles } from 'lucide-react';
...
export default function AskQuestion() {
  ...
  const draftWithAi = async () => {
    setAiLoading(true);
    try {
      const res = await axios.post('/api/ai/draft-question', {
        topic: form.title || form.tags,
        details: form.body
      });
      setForm(f => ({
        ...f,
        title: res.data.title || f.title,
        body: res.data.body || f.body
      }));
      toast.success('Draft filled — edit before posting');
    } catch (err) {
      toast.error(err.response?.data?.message || 'AI draft unavailable');
    } finally {
      setAiLoading(false);
    }
  };
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div className="glass-card" style={{ padding: 28 }}>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 12 }}>
            <button type="button" className="btn btn-secondary btn-sm" onClick={draftWithAi} disabled={aiLoading}>
              <Sparkles size={14} /> {aiLoading ? 'Drafting…' : 'AI draft structure'}
            </button>
          </div>
