...
(`index.js`, using `scripts/verify-functions.js`), a deployment whose database
is missing any of the three race-guard functions refuses to serve. So the
fallbacks should, from here on, be **unreachable in production**.
...
- **With the fallback, today:** if a function is somehow dropped or a migration
  is rolled back, the boot check fails the deploy loudly — but if a function
  vanishes *at runtime* on an already-booted instance, the per-request fallback
  still degrades to the racy path rather than 500-ing every vote. Degraded, not
  down.
...
that has not yet been watched doing its job.
